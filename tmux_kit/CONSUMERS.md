...
| `tmux_kit.labels` | (0.3.3, new — second-consumer contribution from concern-sessions) `label_session()` / `label_sessions()` — pane-harness detection: which agent harness (`amplifier`, `claude-code`, `codex`, or the honest `unknown`) runs in a session's active pane, plus `HarnessLabel` (label + `source` + `evidence`), `HARNESS_UNKNOWN`, and the caller-replaceable `DEFAULT_PROC_BASENAMES` / `DEFAULT_SNAPSHOT_PATTERNS` tables. Process-tree-first (exact-basename match against the EXECUTABLE POSITION only, never a bare argv substring scan), narrow line-anchored snapshot sniff as fallback, never a guess — a wrong label is worse than no label. `label_sessions()` pays for one shared `ps` pass across many sessions. |
